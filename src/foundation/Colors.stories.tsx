...
export const AssistiveColors: StoryObj = {
  render: () => {
    const assistiveColors = [
      { name: "assistive-light", var: "--color-assistive-light", value: "#ffd1d3" },
      { name: "assistive-normal", var: "--color-assistive-normal", value: "#ffa0a3" },
      { name: "assistive-dark", var: "--color-assistive-dark", value: "#e9494e" },
    ];

    return (
      <div className="space-y-6">
        <div>
          <h2 className="heading-lg mb-4">Assistive Colors</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {assistiveColors.map((color) => (
              <div key={color.name} className="border border-gray-200 rounded-lg p-4">
                <div className="w-full h-24 rounded-md mb-3" style={{ backgroundColor: color.value }} />
                <div className="space-y-1">
                  <div className="font-semibold">{color.name}</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {labelColors.map((color) => (
              <div key={color.name} className="border border-gray-200 rounded-lg p-4">
                <div
                  className="w-full h-24 rounded-md mb-3 flex items-center justify-center"
                  style={{ backgroundColor: "#fff", border: `2px solid ${color.value}` }}
                >
                  <span className="font-semibold" style={{ color: color.value }}>
                    샘플 라벨
                  </span>
                </div>
                <div className="space-y-1">
...
export const StaticColors: StoryObj = {
  render: () => {
    const staticColors = [
      ...
      { name: "material-dimmer", var: "--color-material-dimmer", value: "rgba(0, 0, 0, 0.6)" },
      { name: "social-kakao", var: "--color-social-kakao", value: "#ffed47" },
      { name: "interaction-disable", var: "--color-interaction-disable", value: "#eaebec" },
    ];
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {staticColors.map((color) => (
              <div key={color.name} className="border border-gray-200 rounded-lg p-4">
                ...
                >
                  {color.value === "#000" || color.value === "rgba(0, 0, 0, 0.6)" ? (
                    <span className="text-white font-semibold">샘플</span>
                  ) : (
                    <span className="text-gray-800 font-semibold">샘플</span>
                  )}
                </div>
                <div className="space-y-1">
                  <div className="font-semibold">{color.name}</div>
